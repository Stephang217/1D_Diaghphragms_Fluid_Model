...
the profile is still a recognisable kink --- Figure~\ref{fig:front-regimes} shows the
$\tanh$ passing through the lattice points at every width --- but it is sampled too
sparsely for its gradient to be measured, and the gradient integral is the one
quantity (10) needs. Below that the front stops being a travelling profile at all:
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/front_regimes.png}
  \caption{The same front at four couplings, showing how the travelling profile is
  lost. Markers are diaphragms, the line is $\tanh(\xi/\sqrt{2\kappa})$ with nothing
  fitted, and the enlarged markers are the sites lying inside the switch. At
  $\kappa = 8$ ($w = 4.00$) nine diaphragms span the switch and equation~(10) is exact
  to $0.1\%$; at $\kappa = 2$ ($w = 2.00$) four remain and it is good to $1.2\%$; at
  $\kappa = 0.79$ ($w = 1.26$) only three remain, the gradient integral drifts by
  $3.6\%$ within a single run, and the formula is $10\%$ out; at $\kappa = 0.20$
  ($w = 0.63$) the front is frozen between two sites. Note that the $\tanh$ keeps
  passing through the points throughout --- the shape is not what fails first.
  $\Pi = 0.115$, $\Omega = 10$, $\theta = 0.1$, $\Delta\tilde t = 5\times10^{-3}$,
  $N$ and $T$ sized per panel so the front travels $25$ times its own width.}
  \label{fig:front-regimes}
\end{figure}
