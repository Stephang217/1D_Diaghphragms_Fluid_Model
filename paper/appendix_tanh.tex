...
%
\begin{equation}
  v^\ast = \frac{\Delta\tilde\psi}{\Omega I}
...
zero-drive $\tanh$. The correction must be \emph{even} in $\Pi$: sending
$\Pi \to -\Pi$ maps the potential to its mirror image and the kink to its mirror
image, which has the same width. The leading term is therefore $O(\Pi^2)$, and
fitting the measured widths over the grid of Appendix \ref{app:closed-form} gives
%
\begin{equation}
  w \simeq \sqrt{2\bigl(\kappa - v^{2}\bigr)}\;\bigl(1 + 1.53\,\Pi^{2}\bigr).
  \label{eq:width-corrected}
\end{equation}
%
Over the $160$ points that expression predicts the measured width
$w_{\rm eff} = 4/(3I)$ to a median of $1.9\%$ and a worst case of $4.8\%$, against
a worst case of $12.0\%$ for the uncorrected $\sqrt{2\kappa}$.
