...
\begin{figure}[htbp]
  ...
  $v^\ast = 0.068571$, agreeing to $4\times10^{-4}\%$, the two displaced ones
...
$\Omega \in [1, 30]$, at $\theta = 0.1$: the energy balance evaluated with the
\emph{measured} gradient integral, $v_{\rm EB}/v_{\rm meas}$, and the tanh substitution
on top of it, $v_{\rm closed}/v_{\rm EB}$. Reporting only the combined error would say
where \eqref{eq:closed-form} is wrong without saying why.
...
even an approximation here. With the gradient integral taken at 4th order,
$v_{\rm EB}/v_{\rm meas}$ has median $1.0003$ across all $160$ points and never departs
from $1$ by more than $0.45\%$, flat in $\Pi$, $\Omega$ and $w$ alike. The whole of the drift is in the closure, $v_{\rm closed}/v_{\rm EB}$ running
from $1.003$ at $\Pi = 0.02$ to $0.946$ at $\Pi = 0.30$. Nor is it $\Delta\tilde\psi$:
...
\textbf{A correction to an earlier draft of this appendix.} It reported
$v_{\rm EB}/v_{\rm meas}$ falling from $1.032$ at $w = 3$ sites to $1.001$ at $w = 8$, and
